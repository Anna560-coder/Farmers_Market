...
import 'package:farmers_1/Views/Role_based_login/Admin/Model/add_items_model.dart';
...
class AddItemNotifier extends StateNotifier<AddItemState> {
  ...
  Future<void> uploadAndSaveItem(String name, String price) async {
    ...
    print("DEBUG → image: ${state.imagePath}");
    print("DEBUG → category: ${state.selectedCategory}");
    print("DEBUG → sizes: ${state.sizes}");
    print("DEBUG → colors: ${state.colors}");
    print("DEBUG → discount: ${state.discountPercentage}");
    ...
    try {
      ...
      try {
        ...
        await callable.call({'productName': name});
      } catch (_) {}
